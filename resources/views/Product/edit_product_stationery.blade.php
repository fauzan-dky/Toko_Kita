...
@section('toko_kita')
	<div class="container">
		<div class="card bg-warning" style="padding: 20px; margin: 10px;">
			<h1 class="card-tittle text-center">Ubah Produk</h1>
			<div class="card-body">
				<form action="{{url('/save_edited_product_stationery')}}" method="post" enctype="multipart/form-data">
					@csrf
					<label for="nama_produk">Nama:</label>
					<input type="text" class="form-control" id="nama_produk" name="nama_produk" placeholder="Nama Produk" value="{{$list->nama_produk}}">
					
					<label for="harga_satuan">Harga Satuan:</label>
					<input type="number" class="form-control" id="harga_satuan" name="harga_satuan" placeholder="Harga Satuan" value="{{$list->harga_satuan}}">
					
					<label for="stok">Stok:</label>
					<input type="number" class="form-control" id="stok" name="stok" placeholder="Jumlah Stok" value="{{$list->stok}}">
					...
					<textarea id="deskripsi" name="deskripsi" rows="5" cols="20" class="form-control" placeholder="Tambah deskripsi">{{$list->deskripsi}}</textarea>
					<br>
					<input type="text" value="{{$list->id}}" name="id" hidden="">
					
					<button class="btn btn-success" type="button" onclick="validasiData()"><i class="fa fa-share-square-o fa-lg"></i> Kirim</button><!--Validasi Form-->
					<button id="submitData" class="btn btn-success" type="submit" hidden>Kirim</button>
					<a href="{{url('/list_stationery_product')}}" class="btn btn-primary">Kembali</a>
				</form>
			</div>
		</div>
	</div>
